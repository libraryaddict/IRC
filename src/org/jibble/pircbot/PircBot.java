...
public abstract class PircBot implements ReplyConstants {
	...
	private static String returnFormatted(String msg, boolean convertToMC) {
		if (convertToMC == false) {
			for (int i = 0; i <= 21; i++)
				msg = msg.replace(MC[i], IRC[i]);
		} else {
			for (int i = 0; i <= 21; i++) {
				msg = msg.replace(IRC[i], MC[i]);
			}
		}
		return msg;
	}
	...
	public PircBot() {
		for (int i = 0; i <= 9; i++)
			MC[i] = "§" + i;
		for (int i = 10; i <= 15; i++)
			MC[i] = "§" + "abcdef".substring(i - 10, i - 9);
		MC[16] = "§k";
		MC[17] = "§l";
		MC[18] = "§m";
		MC[19] = "§n";
		...
		MC[21] = "§r";
		IRC[0] = Colors.BLACK;
		IRC[1] = Colors.DARK_BLUE;
		IRC[2] = Colors.DARK_GREEN;
		IRC[3] = Colors.TEAL;
		IRC[4] = Colors.BROWN;
		IRC[5] = Colors.PURPLE;
		IRC[6] = Colors.OLIVE;
		IRC[7] = Colors.LIGHT_GRAY;
		IRC[8] = Colors.DARK_GRAY;
		IRC[9] = Colors.BLUE;
		IRC[10] = Colors.GREEN;
		IRC[11] = Colors.CYAN;
		IRC[12] = Colors.RED;
		IRC[13] = Colors.MAGENTA;
		IRC[14] = Colors.YELLOW;
		IRC[15] = Colors.WHITE;
		IRC[16] = "§k";
		IRC[17] = Colors.BOLD;
		IRC[18] = "§m";
		IRC[19] = Colors.UNDERLINE;
		IRC[20] = "";
		IRC[21] = Colors.NORMAL;
	...
	public final void sendMessage(String target, String message) {
		...
		if (isDelay())
			_outQueue.add("PRIVMSG " + target + " :"
					+ returnFormatted(message, false));
		else
			sendRawLine("PRIVMSG " + target + " :"
					+ returnFormatted(message, false));
	...
	public final void sendAction(String target, String action) {
		...
		sendCTCPCommand(target, "ACTION " + returnFormatted(action, false));
	...
	public final void sendNotice(String target, String notice) {
		...
		if (!isDelay())
			_outQueue.add("NOTICE " + target + " :"
					+ returnFormatted(notice, false));
		else
			sendRawLine("NOTICE " + target + " :"
					+ returnFormatted(notice, false));
	 ...
	 * @return Prefix
	 */
	public final String getPrefix(String channel, String nick) {
		String prefix = "";
	...
	public final void kick(String channel, String nick, String reason) {
		...
		this.sendRawLine("KICK " + channel + " " + nick + " :"
				+ returnFormatted(reason, false));
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0
			...
			if (request.equals("VERSION")) {
				...
				this.onAction(sourceNick, sourceLogin, sourceHostname, target,
						returnFormatted(request.substring(7), true));
			...
			this.onMessage(
					...
					sourceLogin,
					sourceHostname,
					returnFormatted(line.substring(line.indexOf(" :") + 2),
							true));
		} else if (command.equals("PRIVMSG")) {
		...
		if (code == RPL_LIST) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				if (nick.startsWith("@")) {
					...
					// User is half admin in this channel.
					prefix = "&";
				} else if (nick.startsWith("%")) {
					// User is halfop in this channel.
					prefix = "%";
				} else if (nick.startsWith("!")) {
					// User is something in this channel.
					prefix = "!";
				}
	 ...
	 * @return User
	 */
	public final User getUser(String channel, String name) {
